...
Controllers.Layers = {
  ...
  mark_layer_as_unloaded: function(cp, layer_default_id) {
    var params_key = stringify(active_layer.parameters);
    cp.set("layers_loading_states."+layer_default_id + "." + params_key, false);
  },
  mark_layer_as_loaded: function (cp, active_layer, success, force_reload) {
    var params_key = stringify(active_layer.parameters);
    var this_layer = cp.get("layers_loading_states." + active_layer.layer_default_id + "." + params_key) || { loaded: 0, errors: 0};
